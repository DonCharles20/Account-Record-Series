...
private static final long serialVersionUID = 3417040053089159204L;
   ...
   public ReadSequentialFile()
   {
      ...
      userInterface = new BankUI(9);                    //STEP 8 
      getContentPane().add( 
         userInterface, BorderLayout.CENTER );
      for(int x = 0; x < 9; x++){
    	  userInterface.fields[x].setEditable( false );
    	  userInterface.fields[x].setBackground(Color.yellow);
      }
      // get reference to generic task button doTask1 from BankUI
      openButton = userInterface.getDoTask1Button();
      openButton.setText( "Open File" );
      openButton.setMnemonic('o');  //Remember, accelerators should be a capital!!!!!
      ...
      try {
         ...
         String values[] = {
            ...
            record.getLastName(),
			   record.getAddress(),
            record.getSocSec(),	
            String.valueOf(record.getSex()),
            record.getTitle(),   
            String.valueOf(record.getGpa()),
            String.valueOf(record.getBalance())	//STEP 9
			};
